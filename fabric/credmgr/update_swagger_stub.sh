...
find ${STUB_DIR}/swagger_server \( -type d -name .git -prune \) -o -type f -print0 | xargs -0 sed -i '' 's/swagger_server/fabric.credmgr.swagger_server/g'
...
while read f; do
  ...
  sed -i '' "s/from fabric.credmgr.swagger_server import util/from fabric.credmgr.swagger_server import util\\"$'\n'\\"from fabric.credmgr.swagger_server.response import ${f%???} as rc/g" \

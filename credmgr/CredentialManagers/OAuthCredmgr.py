...
class OAuthCredmgr(AbstractCredentialManager):
    ...
    def should_delete(self, username, token_name):
        ...
        if os.path.exists(mark_path):
            ...
            # if mark file is older than 24 hours (or oauth-credmgr-token-lifetime if defined), delete tokens
            self.log.debug('Mark file is %d seconds old', int(time.time() - mtime))
            if CONFIG is not None and 'oauth-credmgr-token-lifetime' in CONFIG:
                if time.time() - mtime > int(CONFIG.get("oauth", "oauth-credmgr-token-lifetime")):
                    return True
